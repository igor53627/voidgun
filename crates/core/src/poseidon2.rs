...
fn field_from_hex(hex: &str) -> Field {
    use ark_ff::PrimeField;
    Field::from_be_bytes_mod_order(&hex::decode(hex).expect("valid hex"))
}

pub static INTERNAL_MATRIX_DIAGONAL: Lazy<[Field; 4]> = Lazy::new(|| {
    [
...
pub static ROUND_CONSTANTS: Lazy<[[Field; 4]; 64]> = Lazy::new(|| {
    [
        [
...
pub fn sbox(x: Field) -> Field {
    let x2 = x.square();
    let x4 = x2.square();
    x4 * x
}

pub fn matrix_multiplication_4x4(state: &mut [Field; 4]) {
    let t0 = state[0] + state[1];
    let t1 = state[2] + state[3];
    let mut t2 = state[1].double();
    t2 += t1;
    let mut t3 = state[3].double();
    t3 += t0;
    let mut t4 = t1.double();
    t4 = t4.double();
    t4 += t3;
    let mut t5 = t0.double();
    t5 = t5.double();
    t5 += t2;
    let t6 = t3 + t5;
    let t7 = t2 + t4;
...
pub fn internal_matrix_multiplication(state: &mut [Field; 4]) {
    let sum: Field = state.iter().sum();
    for (i, s) in state.iter_mut().enumerate() {
        *s = *s * INTERNAL_MATRIX_DIAGONAL[i] + sum;
    }
}

pub fn permutation(state: &mut [Field; 4]) {
    matrix_multiplication_4x4(state);

    let rf_first = ROUNDS_F / 2;
    for r in 0..rf_first {
        for (s, c) in state.iter_mut().zip(ROUND_CONSTANTS[r].iter()) {
            *s += c;
        }
...
mod tests {
    ...
    #[test]
    fn test_zero_permutation() {
        let mut state = [Field::ZERO; 4];
        permutation(&mut state);
        ...
        assert_eq!(state, expected, "Zero permutation output mismatch");
    }
    ...
    fn test_sponge_hash_basic() {
        ...
        assert_ne!(
            result,
            Field::ZERO,
            "Sponge hash should produce non-zero output"
        );
    }
    ...
    fn test_cross_language_vectors() {
        ...
        let expected_merkle_00 =
            field_from_hex("1218536453df604871fd18460a1d5c2abf9d9cfcda586312bfc3b78d75e29cf0");
        let expected_merkle_123_456 =
            field_from_hex("24cbf5ece05503c37381b5d7dfcaf96fe2aca3749cb1a5d4d2f5264e40872fa2");
        let expected_hash_1_2_3 =
            field_from_hex("23864adb160dddf590f1d3303683ebcb914f828e2635f6e85a32f0a1aecd3dd8");

        assert_eq!(
            merkle_00, expected_merkle_00,
            "merkle_node(0,0) mismatch - Solidity/Noir must match"
        );
        assert_eq!(
            merkle_123_456, expected_merkle_123_456,
            "merkle_node(123,456) mismatch"
        );
        assert_eq!(
            hash_1_2_3, expected_hash_1_2_3,
            "sponge_hash([1,2,3]) mismatch"
        );
        assert_eq!(
            hash_domain_2_0_0, expected_merkle_00,
            "sponge_hash([2,0,0]) should equal merkle_node(0,0)"
        );
    }
}
